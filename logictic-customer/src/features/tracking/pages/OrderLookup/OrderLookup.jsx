...
} from "@features/tracking/api/publicParcelTrackingApi";
...
/*
 * Kiểu chip theo mã trạng thái đơn. Đơn ký gửi dùng 19 mã đích của module
 * trạng thái dùng chung (mã cũ được chuẩn hóa trước khi tra bảng này); đơn mua
 * hộ dùng lại cùng mã PENDING_REVIEW / APPROVED / COMPLETED...
 */
const STATUS_STYLE = {
  PENDING_REVIEW: {
...
const normalizeTrackingResult = (
  ...
  /*
   * Đơn ký gửi: chuẩn hóa về mã đích và lấy nhãn từ module dùng chung để
   * trang tra cứu nói cùng một chữ với màn "Đơn của tôi". Đơn mua hộ có máy
   * trạng thái riêng nên giữ nhãn hệ thống trả về.
   */
  const isPurchaseOrder =
    normalizeStatus(data.orderType) === "PURCHASE";

  const status = isPurchaseOrder
    ? normalizeStatus(data.status)
    : normalizeOrderStatus(normalizeStatus(data.status)) || "";
                  ...
                  >
                    ...
                    <div className="order-lookup-detail-layout">
                      ...
                      <aside className="order-lookup-side">
                        ...
                        {/* Tra cứu công khai chỉ có trạng thái. Hành trình chi tiết (chặng, chuyến,
                            mã vận đơn hãng, ngày dự kiến) cần đăng nhập — dẫn khách sang đó. */}
                        <div className="order-lookup-note-card">
                          <ArrowRightOutlined />

                          <p>
                            Muốn xem hành trình chi tiết, mã vận đơn của hãng và ngày dự kiến về?{" "}
                            <Link
                              to={`/tracking?search=${encodeURIComponent(
                                orderData.orderCode
                              )}&finished=1`}
                            >
                              Đăng nhập và mở Theo dõi đơn hàng
                            </Link>
                          </p>
                        </div>
                      </aside>
