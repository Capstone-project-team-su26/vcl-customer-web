/* =========================================================
   purchaseRequestApi.mock.js — BẢN MOCK, giữ lại cho các màn chưa nối API thật.
...
const buildMockCheckoutUrl = (purchaseRequestId, orderCode) => {
  ...
  /* Chuỗi URL, không phải hợp đồng API: bám theo route mới của màn báo giá mua hộ
     (/orders/mua-ho/{id}/bao-gia). Route cũ vẫn chuyển hướng nhưng sẽ rụng mất query. */
  return `${origin}/orders/mua-ho/${encodeURIComponent(
    purchaseRequestId
  )}/bao-gia?orderCode=${encodeURIComponent(orderCode)}`;
};
