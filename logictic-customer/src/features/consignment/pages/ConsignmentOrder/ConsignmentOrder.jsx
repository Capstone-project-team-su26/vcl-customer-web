...
export default function ConsignmentOrder() {
  ...
  /* Dịch vụ chọn theo từng kiện, phụ thuộc tuyến + phương án vận chuyển. */
  const [itemServiceOptions, setItemServiceOptions] = useState([]);
  const [isLoadingItemServices, setIsLoadingItemServices] = useState(false);
  const [itemServicesError, setItemServicesError] = useState("");
  ...
  useEffect(() => {
    /*
     * Chưa đủ tuyến + phương án thì PackageItemServices hiện câu nhắc (needsRoute),
     * danh sách cũ không hiển thị; chọn lại sẽ tải mới.
     */
    if (!form.route || !form.shippingOption) {
      return undefined;
    }

    const controller = new AbortController();

    const loadItemServices = async () => {
      try {
        setIsLoadingItemServices(true);
        setItemServicesError("");

        const services = await getConsignmentItemServicesApi(
          {
            route: form.route,
        ...
        setItemServiceOptions(services);

        /*
         * Đổi tuyến/phương án có thể làm dịch vụ đã chọn không còn áp dụng
         * (backend trả 400 nếu gửi lên). Bỏ các dịch vụ đó và báo khách.
         */
        const validIds = new Set(
          services.map((service) => service.pricingRuleId),
        );
        const hasInvalidSelection = packagesRef.current.some((pkg) =>
          (pkg.serviceIds || []).some((id) => !validIds.has(id)),
        );

        setPackages((previous) =>
          previous.map((pkg) => {
            const current = pkg.serviceIds || [];
        ...
        if (hasInvalidSelection) {
          AuthNotify.warning(
            "Đã bỏ dịch vụ không còn áp dụng",
            "Một số dịch vụ đã chọn không áp dụng cho tuyến hoặc hình thức vận chuyển mới. Vui lòng kiểm tra lại dịch vụ của từng kiện.",
          );
        }
      } catch (error) {
        ...
        setItemServiceOptions([]);
        setItemServicesError(
          getApiErrorMessage(error, "Không thể tải danh sách dịch vụ."),
        );
      } finally {
        if (!controller.signal.aborted) {
          setIsLoadingItemServices(false);
        }
      }
    };
  ...
  }, [form.route, form.shippingOption]);
  ...
  const handleCreateOrder = async () => {
    ...
    try {
       ...
       * xác nhận được; báo giá chính thức do Sale gửi sau.
       */
      const estimatedTotal = Number(
        created?.quotation?.totalEstimatedCost,
      );

      AuthNotify.success(
        "Tạo đơn thành công",
        Number.isFinite(estimatedTotal) && estimatedTotal > 0
          ? `Đơn ${created?.consignmentCode || ""} đã được tiếp nhận. Giá tạm tính ${formatVnd(Math.round(estimatedTotal))} đ (tham khảo) — nhân viên sẽ gửi báo giá chính thức.`
          : "Đơn hàng ký gửi đã được tiếp nhận.",
      );

      /* warnings[]: hàng thuộc danh mục hạn chế { productName, matchedItem, restrictionType, note } — vẫn tạo được đơn. */
      const warnings = (Array.isArray(created?.warnings) ? created.warnings : [])
        .map((warning) => {
          ...
          const name = warning?.productName || warning?.matchedItem || "";
          const detail = warning?.note || warning?.matchedItem || "";

          return name
            ? `${name}: hàng hạn chế${detail && detail !== name ? ` (${detail})` : ""}`
            : detail;
        })
        .filter(Boolean);
  ...
  if (isConfirming) {
    return (
      <ConsignmentOrderConfirm
        ...
        itemServiceOptions={itemServiceOptions}
          ...
          <div className="scrollable-content-wrapper">
              ...
              return (
                ...
                >
                  ...
                  <PackageItemServices
                    ...
                    disabled={isSubmitting}
                    onChange={(nextIds) =>
                      handleInputChange(pkg.id, "serviceIds", nextIds)
                    }
                  />
            ...
            >
              ...
              <PackageOptionalServices
                ...
                disabled={isSubmitting}
                triggerTitle="Đóng thùng gỗ cho từng kiện"
                triggerDescription="Bắt buộc: chọn cấu hình thùng gỗ cho mỗi kiện. Dịch vụ khác (kiểm hàng, bảo hiểm...) chọn trong từng kiện."
                modalEyebrow="ĐÓNG THÙNG GỖ"
                modalTitle="Chọn cấu hình thùng gỗ cho từng kiện"
                modalDescription="Đóng thùng gỗ là bắt buộc và phải có một cấu hình kích thước cho từng kiện. Phí thùng tính theo cấu hình của kiện."
