import {
  ...
  export default function ServicePolicyDetail({
    open,
    pricing,
    onClose,
  }) {
    ...
    useEffect(() => {
      ...
      const fetchDetail = async () => {
        try {
          ...
          const response = await getServicePricingById(
            pricingId,
            {
    ...
    const fields = useMemo(() => {
      ...
      const orderedKeys = [
        ...PRIORITY_FIELDS.filter((key) =>
          keys.includes(key)
        ),
        ...keys.filter(
          (key) => !PRIORITY_FIELDS.includes(key)
        ),
      ];
        ...
        >
          ...
          <div className="service-policy-detail__hero">
            <div>
              ...
              <small>
                Trên mỗi {detail?.unitType || "đơn vị"}
              </small>
            </div>
